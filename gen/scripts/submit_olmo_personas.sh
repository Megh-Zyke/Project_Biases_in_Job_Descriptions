#!/bin/bash
#SBATCH --job-name=generate_personas_olmo
...
#SBATCH --nodes=1
#SBATCH --output=generate_personas_olmo_%j.out
...
OUT_DIR="gen/out/personas_output_olmo"
...
if [ -n "$USER_OUT_DIR" ]; then
    ...
    OUT_DIR="gen/out/personas_output_olmo_${SLURM_JOB_ID:-temp}_${START_ROW}_${END_ROW}"
fi
...
while [ $CURRENT_START -lt $END_ROW ]; do
  ...
  if [ $CURRENT_START -eq $START_ROW ] && [ "$APPEND_FIRST" != "true" ]; then
    ...
    python persona_generation/persona_generation.py \
      ...
      2>&1 | tee -a "logs/chunk_olmo_$CHUNK_NUM.log"
  else
  ...
  if [ ${PIPESTATUS[0]} -ne 0 ]; then
    echo "ERROR: Chunk $CHUNK_NUM failed! Check logs/chunk_olmo_$CHUNK_NUM.log"
    exit 1
  fi
...
echo "Output files:"
echo "  CSV:  $OUT_FILE"
echo "Logs:   logs/chunk_olmo_*.log"
echo "========================================================================"
